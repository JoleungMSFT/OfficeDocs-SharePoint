...
|SharePoint OnPrem Prioritization   <br/> |For hybrid scenarios where the email is the same for both SharePoint Server on-premises and SharePoint in Microsoft 365, determines whether or not the client should set up sync for SharePoint Server or SharePoint in Microsoft 365 first during the first-run scenario.   <br/> |SharePointOnPremPrioritizationPolicy (int): This parameter determines which service to attempt to authenticate against for setting up sync.<br/> 1 indicates OneDrive should set up SharePoint Server on-premises first, followed by SharePoint in Microsoft 365.  <br/> |\<key\>SharePointOnPremPrioritizationPolicy</key\> <br/> \<int\>(0 or 1)</int\> <br/> | 
...
|Disable Report a Problem|This setting prevents users from contacting Microsoft's customer support or opening tickets to track issues.|isReportAProblemDisabled (Bool): If set to true, all options to contact Microsoft's customer support will be removed.|\<key\>isReportAProblemDisabled\</key\><br/>\<(Bool)/\>|
